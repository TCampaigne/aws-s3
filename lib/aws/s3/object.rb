    ...
    class S3Object < Base
      class << self
        ...
        def find(key, bucket = nil)
          ...
          # Bucket marker has to be unescaped to find the S3 Object
          bucket = Bucket.find(bucket_name(bucket), :marker => URI.unescape(key.previous), :max_keys => 1)

          # If our heuristic failed, trigger a NoSuchKey exception
          if (object = bucket.objects.first) && URI.escape(object.key) == key # We assume key is escaped
            # The object key is not escaped, but needs to be so that URI doesn't throw an error for an invalid URI
            object.key = URI.escape(object.key)
            object
          else
